...
def parse_arguments():
  ...
  parser.add_argument('-r', '--random', help="Repeat for n random iterations", action='store_true')
...
def get_config(config_file, exp_dir=None, is_test=False):
  ...
  config.save_dir = os.path.join(config.experiment.dir, config.experiment.name)
  save_name = os.path.join(config.save_dir, 'config.yaml')

  # snapshot hyperparameters
  mkdir(config.experiment.dir)
